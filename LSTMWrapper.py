...
class LSTM(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        lstm_out = lstm_out.contiguous().view(-1, (self.bidirectional + 1) * self.hidden_dim)
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = (weight.new((self.bidirectional + 1) * self.n_layers, batch_size, self.hidden_dim).zero_().to(device),
                  weight.new((self.bidirectional + 1) * self.n_layers, batch_size, self.hidden_dim).zero_().to(device))
        return hidden
...
def fit(x, y, model, opt, loss_fn, batch_size, decay_rate):
    ...
    while not epochs_exceeded:
        ...
        if current_loss < best_loss:
            ...
            print("SAVE")
            torch.save(model.state_dict(), "lstm_age_model_state.pt")
...
def LSTM_run(hidden_size, learning_rate, batch_size, num_layers, bidirectional, decay_rate):
    #hidden_size = 128
    #num_epochs = 300
    #learning_rate = 0.1
    #batch_size = 10
    ...
    print('Author accuracy before training', author_accuracy(y_pred, test_tweet_numbers, 0))
    ...
    print('Author accuracy after training', author_accuracy(y_pred, test_tweet_numbers, 0))
